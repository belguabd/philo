...
/*   Updated: 2024/05/25 15:00:22 by belguabd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&philo->mtr->check_is_died);
	if (!philo->mtr->stop_simulation)
	{
		pthread_mutex_unlock(&philo->mtr->check_is_died);
		return (0);
	}
	pthread_mutex_unlock(&philo->mtr->check_is_died);
	return (-1);
