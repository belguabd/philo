...
/*   Updated: 2024/06/03 15:43:34 by belguabd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/philo_bonus.h"

void	init_semaphores(t_data **data, int num_philo)
{
	sem_unlink("/fork");
	...
	if ((*data)->stop_eat_lock == SEM_FAILED)
		ft_putendl_fd("fail sem_open", 2);
	(*data)->philos = (t_philo *)malloc(sizeof(t_philo) * num_philo);
	if (!(*data)->philos)
		ft_putendl_fd("fail malloc", 2);
}
...
void	init_philos(t_data **data)
{
	...
	while (i < (*data)->num_philo)
	{
		...
		(*data)->philos[i].start = ft_get_current_time();
...
void	init_data(t_data **data, char *av[])
{
	...
	init_semaphores(data, num_philo);
	init_philos(data);
}
